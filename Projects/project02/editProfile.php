<?php
    ...
    */
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!$error) 
        {
            if (!empty($first_name) && !empty($last_name) && !empty($gender) && !empty($birthday) && !empty($weight)) 
            {
                // Only set the picture column if there is a new picture
                if (!empty($new_picture)) 
                {
                    $query = "UPDATE exercise_user SET first_name = '$first_name', last_name = '$last_name', gender = '$gender', " .
                        " birthday = '$birthday', weight = '$weight', picture = '$new_picture' WHERE userID = '" .$_SESSION['userID']."'";
                }
                else 
                {
                    $query = "UPDATE exercise_user SET first_name = '$first_name', last_name = '$last_name', gender = '$gender', " .
                        " birthday = '$birthday', weight = '$weight' WHERE userID = '".$_SESSION['userID']."'";
                }
                mysqli_query($dbc, $query)
                        or die('Error updating DB');
    ...
    else 
    {
        ...
        $query = "SELECT first_name, last_name, gender, birthday, weight, picture FROM exercise_user WHERE userID = '".$_SESSION['userID']."'";
...
?>
    ...
    <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        ...
        <fieldset>
            ...
            <input type="date" id="birthday" name="birthday" value="<?php if (!empty($birthday)) echo $birthday; else echo 'YYYY-MM-DD'; ?>" /><br />
