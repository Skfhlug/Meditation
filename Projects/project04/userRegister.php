    ...
    if (isset($_POST['submit'])) 
    {
        ...
        if (!empty($username) && !empty($password1) && !empty($password2) && ($password1 == $password2)) 
        {
            ...
            if (mysqli_num_rows($data) == 0) 
            {
                ...
                $query = "INSERT INTO user (userID, username, password, firstName, lastName, address, state, zip, phone, email, registerdate, userPrivilege)".
                    " VALUES (0,'$username', SHA('$password1'),'$firstName', '$lastName', '$address','$state','$zip', '$phone', '$email',now(),1);";
                ...
                echo '<a href="login.php">Click here Log in</a>';
                mysqli_close($dbc);
                exit();
...
<html>
    <body>
        ...
        <p>Please enter your username and desired password to sign up to be Inner Peace Member.</p>
        <form class="form-group" method="post" style="margin:auto; width: 500px;"action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <fieldset>
        <legend>Registration Info for new User</legend>
